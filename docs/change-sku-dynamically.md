...
[![VCT Change SKU Dynamically Logo](../static/img/docs/Vct_ChangeSkuDynamically.svg)](https://commercemarketplace.adobe.com/vct-changeskudynamically.html)

- [Marketplace Page](https://commercemarketplace.adobe.com/vct-changeskudynamically.html)
...
```bash
composer require vct/changeskudynamically # Install module with Composer
...
It is necessary to specify the correct selector for your frontend theme to display selected product data instead of the configurable product data on the configurable product page, when an option is selected.
...
![Frontend selector devtools](../static/img/docs/change-sku-dynamically/frontend-selector-devtools.png)
...
For example element selector for the product SKU is `.product.attribute.sku .value`:
![Selector validation in Admin](../static/img/docs/change-sku-dynamically/adminhtml-selector-validation.png)
where `..value` is an invalid jQuery selector. Use `.value` instead of  `..value` in this example.

## Example

![Example](../static/img/docs/change-sku-dynamically/frontend-sku.png)

##### Use <kbd>Product Info Switcher</kbd> module to switch other elements
