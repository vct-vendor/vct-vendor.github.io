...
```bash
composer update vct/simpleproducturl # Update the module using Composer
bin/magento setup:upgrade --safe-mode=1 # Update the database schema and data
bin/magento setup:static-content:deploy --force # Deploy static view files
```
...
- A product has the URL `/mona-pullover-hoodie-s-green.html`.
- This product is one of the variants of the configurable product with URL `/mona-pullover-hoodie.html`.
